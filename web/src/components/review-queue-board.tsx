...
function AssignmentFeedback({ state }: { state: ReviewAssignmentState }) {
  ...
  return <div className={state.ok ? "batch-feedback success" : "batch-feedback error"} role="status"><strong>{state.message}</strong>{state.results.length > 0 && <ul>{state.results.map((item) => <li key={item.id}><code>{item.id}</code><span className={item.ok ? "action-success" : "action-error"}>{item.ok ? "成功" : "失败"}：{item.message}</span></li>)}</ul>}</div>;
}
...
export function ReviewQueueBoard({ items, tasks, csrfToken, assignAction }: { items: CatalogReviewQueueItem[]; tasks: ReviewTask[]; csrfToken: string; assignAction: ReviewAssignmentAction }) {
  const [selected, setSelected] = useState<string[]>([]);
  const [state, action, pending] = useActionState(assignAction, emptyReviewAssignmentState);
  ...
  return <>
    <section className="review-assignment-panel" aria-label="批量分派审核任务"><header><div><p className="eyebrow">QUEUE ASSIGNMENT</p><h2>批量分派</h2></div><label className="select-all"><input type="checkbox" checked={allSelected} onChange={(event) => setSelected(event.target.checked ? items.map((item) => item.patternId) : [])}/>全选当前 {items.length} 条</label></header><p className="batch-selection">已选择 <strong>{selected.length}</strong> 条。分派仅建立审核任务，不会更改版权状态或发布记录。</p><form action={action} className="review-assignment-form"><input type="hidden" name="csrfToken" value={csrfToken}/>{selected.map((id) => <input key={id} type="hidden" name="selectedIds" value={id}/>)}<label>审核人<input name="assignee" required maxLength={120} placeholder="输入审核人账号或标识"/></label><button type="submit" disabled={pending || selected.length === 0}>{pending ? "分派中…" : "分派选中记录"}</button></form><AssignmentFeedback state={state}/></section>
